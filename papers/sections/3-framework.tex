...
The InsightEval framework implements a four-stage evaluation pipeline. Each stage is modular and can be independently configured or extended.
...
As illustrated in Figure~\ref{fig:interface}, InsightEval provides an intuitive web-based interface that guides users through the entire insight evaluation workflow. The key functionalities are accessible through the following views:

\begin{figure*}[!t]
\centering
    \includegraphics[width=1\textwidth]{figures/InsightEvaluation_05.png}
\caption{The InsightEval user interface. The \textbf{left panel} displays the home page with the evaluation pipeline overview and user input area. The \textbf{right panel} demonstrates the step-by-step analysis results.}\label{fig:interface}
\end{figure*}

(1)~\textbf{Homepage and Input}: Displays the evaluation pipeline overview and provides an upload interface for the target paper PDF and reference PDFs, with real-time progress tracking.

(2)~\textbf{Sentence Extraction View}: Presents extracted sentences with citation detection statistics, classifying each as context or citation sentence for immediate structural overview.

(3)~\textbf{Evidence Retrieval View}: Shows filtered viewpoint sentences paired with retrieved evidence passages, including source attribution and relevance indicators.

(4)~\textbf{Scoring Dashboard}: Visualizes multi-dimensional scores (Depth, Breadth, Height) via radar charts for each viewpoint, accompanied by the LLM's chain-of-thought rationale.

(5)~\textbf{Insight Report}: Generates a comprehensive paper-level report with an overall insightfulness score, executive summary, and structured strengths-and-weaknesses analysis.
